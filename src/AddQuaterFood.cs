    ...
    public static class AddQuaterFood
    {
        ...
        private static void Player_AddFood(On.Player.orig_AddFood orig, Player self, int add)
        {
            ...
            if (ModManager.CoopAvailable
                ...
                && self.abstractCreature.world.game.Players[0] != self.abstractCreature
                && !self.isNPC)
            {
                    ...
                    : self.slugcatStats.maxFood))
            {
                ...
                if (self.playerState is PlayerNPCState npc)
                    npc.Malnourished = false;
            }
        }
        ...
        private static void Player_JollyFoodUpdate(On.Player.orig_JollyFoodUpdate orig, Player self)
        {
                ...
                && !self.isNPC)
            {
                ...
                if (self.abstractCreature.world.game.Players[0]?.realizedCreature is Player mainPlayer)
                {
                    self.playerState.foodInStomach = Mathf.Clamp(
                        mainPlayer.playerState.foodInStomach,
                        0,
                        self.MaxFoodInStomach);

                    self.playerState.quarterFoodPoints =
                        mainPlayer.playerState.quarterFoodPoints;
                }
                else
                {
                    orig(self);
