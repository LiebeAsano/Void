...
public class DartPoison : PhysicalObject, IDrawable
{
    ...
    {
        public static Mode Free = new Mode("Free", register: true);

        public static Mode Shot = new Mode("Shot", register: true);

        public static Mode StuckInChunk = new Mode("StuckInChunk", register: true);

        public static Mode StuckInTerrain = new Mode("StuckInTerrain", register: true);
    ...
    public static int UntilPoisonDelay = 340;
    ...
    public void Shoot(Vector2 pos, Vector2 dir, Creature shotBy)
    {
        ...
        needleDir = dir;
        lastNeedleDir = dir;
        poisonCounter = UntilPoisonDelay;
        newAndPink = 1f;
    ...
    public void ChangeMode(Mode newMode)
    {
        ...
        {
            (shotBy as Dartspider).AI.spitModule.CreatureHitByDart((stuckInChunk.owner as Creature).abstractCreature);
        }
    ...
    public override void Update(bool eu)
    {
        ...
        else if (mode == Mode.StuckInChunk)
        {
            ...
            if (poisonCounter > 0)
            {
                if (poisonCounter < 300)
                {
                    float t = Mathf.InverseLerp(260f, 30f, poisonCounter);
                    ...
                    if (stuckInChunk.owner is Player p && (p.IsVoid() || p.IsVoid()) && UnityEngine.Random.value < 1f / Mathf.Lerp(60f, 10f, t))
                    {
                        //(stuckInChunk.owner as Player).InjectPoison(amount/2, Color.Lerp(this.yellow, new Color(0.5f, 0.5f, 0.5f), 0.025f));
                ...
                if (poisonCounter == 0)
                {
                    ...
                    {
                        (stuckInChunk.owner as Creature).InjectPoison(amount, Color.Lerp(this.yellow, new Color(0.5f, 0.5f, 0.5f), 0.025f * Custom.IntClamp(num, 1, 4)));
