...
public class LWMimicstarfish : Creature, PhysicalObject.IHaveAppendages
{
    ...
    public LWMimicstarfish(AbstractCreature abstractCreature, World world) : base(abstractCreature, world)
    {
        this.world = world;
        ...
        this.eatObjects = new List<LWMimicstarfish.EatObject>();
    ...
    public override void InitiateGraphicsModule()
    {
        ...
        {
            base.graphicsModule = new LWMimicGraphics(this);
        }
    }
    ...
    public override void Update(bool eu)
    {
        ...
        for (int m = 0; m < this.tentacles.Length; m++)
        {
            if (ModManager.MSC)
            {
                if ((base.State as LWMimicstarfish.StarState).tentacleHealth[m] < 1f)
                {

                    (base.State as LWMimicstarfish.StarState).tentacleHealth[m] += 0.001f;

                }

                if ((base.State as LWMimicstarfish.StarState).tentacleHealth[m] > 1f)
                {
                    (base.State as LWMimicstarfish.StarState).tentacleHealth[m] = 1f;
                }
            }
    ...
    public override void Collide(PhysicalObject otherObject, int myChunk, int otherChunk)
    {
        ...
        if (creatureRepresentation != null && this.AI.DynamicRelationship(creatureRepresentation).type == CreatureTemplate.Relationship.Type.Eats && this.CheckDaddyConsumption(otherObject))
        {
            ...
            if (flag && (!base.safariControlled || (base.safariControlled && this.inputWithDiagonals != null && this.inputWithDiagonals.Value.pckp)))
            {
                ...
                this.eatObjects.Add(new LWMimicstarfish.EatObject(otherObject.bodyChunks[otherChunk], Vector2.Distance(this.MiddleOfBody, otherObject.bodyChunks[otherChunk].pos)));
    ...
    public override void Violence(BodyChunk source, Vector2? directionAndMomentum, BodyChunk hitChunk, PhysicalObject.Appendage.Pos hitAppendage, Creature.DamageType type, float damage, float stunBonus)
    {
        if (hitAppendage != null)
        {
            damage /= (2f);
            stunBonus /= (1.2f);
            (base.State as LWMimicstarfish.StarState).tentacleHealth[hitAppendage.appendage.appIndex] -= damage;
    ...
    public bool CheckDaddyConsumption(PhysicalObject otherObject)
    {
        ...
        if (otherObject != null)
        {
            if (otherObject is LWMimicstarfish)
            {
    ...
    public List<LWMimicstarfish.EatObject> eatObjects;
    ...
    public override float VisibilityBonus
    {
        get
        {
            if (base.graphicsModule != null)
            {
                return -(base.graphicsModule as LWMimicGraphics).Camouflaged;
            }
            return 0f;
        }
    }
