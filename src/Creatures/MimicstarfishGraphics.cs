...
internal sealed class MimicstarfishGraphics : DaddyGraphics
{
    ...
    public override void Update()
    {
        ...
        {
            if (star.State.health < 0.6f && Random.value * 1.5f < star.State.health && Random.value < 1f / (this.star.Stunned ? 10f : 40f))
            {
                this.whiteGlitchFit = (int)Mathf.Lerp(5f, 40f, (1f - star.State.health) * Random.value);
            }
    ...
    public abstract class StarTubeGraphic : RopeGraphic
    {
        ...
        public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
        {
            ...
            for (int i = 0; i < this.bump.Length; i++)
            {
                ...
                {
                    scale = Mathf.Lerp(2f, 6f, this.bump[i].size) / 10f
                };
                if (this.bump[i].eyeSize > 0f)
                {
                    sLeaser.sprites[this.firstSprit + 1 + this.bump.Length + num] = new FSprite("Circle20", false)
                    {
                        scale = Mathf.Lerp(2f, 6f, this.bump[i].size) * this.bump[i].eyeSize / 10f
                    };
                    num++;
                }
            }
        ...
        public void Colorleg(RoomCamera.SpriteLeaser sLeaser, Color col)
        {
            ...
            {
                float floatPos = Mathf.InverseLerp(0.3f, 1f, i / (float)((sLeaser.sprites[this.firstSprit] as TriangleMesh).vertices.Length - 1));
        ...
        {
            Vector2 p = this.OneDimensionalTubePo(pos.y - (1f / segments.Length), timeStacker);
            Vector2 p2 = this.OneDimensionalTubePo(pos.y + (1f / segments.Length), timeStacker);
    ...
    public class StarLegGraphic : MimicstarfishGraphics.StarTubeGraphic
    {
        ...
        public StarLegGraphic(DaddyGraphics owner, int tentacleIndex, int firstSprit) : base(owner, (int)(owner.daddy.tentacles[tentacleIndex].idealLength / 10f), firstSprit)
        {
            ...
            for (int i = 0; i < this.bump.Length; i++)
            {
                ...
                bump[i] = new MimicstarfishGraphics.StarTubeGraphic.Bumpo(new Vector2(Mathf.Lerp(-1f, 1f, Random.value) * 3f * num2, Mathf.Lerp(Mathf.InverseLerp(0f, num, num - 20), 1f, num2)), Mathf.Lerp(Random.value, num2, Random.value) * (owner.SizeClass ? 1f : 0.8f), (Random.value < Mathf.Lerp(0f, 0.6f, num2)) ? Mathf.Lerp(0.2f, 0.8f, Mathf.Pow(Random.value, Mathf.Lerp(1.5f, 0.5f, num2))) : 0f);
                sprite++;
        ...
        public override void Update()
        {
            ...
            for (int i = 0; i < this.tentacle.tChunks.Length; i++)
            {
                ...
                {
                    base.AddToPositionsList(listCount++, this.tentacle.tChunks[i].rope.GetPosition(j) + (Custom.RNV() * Mathf.InverseLerp(4f, 14f, tentacle.stun) * 4f * Random.value));
                }
            }
