...
namespace VoidTemplate.Creatures
{
    ...
    public class Outspector : Creature, PhysicalObject.IHaveAppendages, IOwnProjectedCircles
    {
        public class OutspectorState : HealthState
        {
            ...
            public OutspectorState(AbstractCreature creature)
                : base(creature)
            {
        ...
        public Color OwneriteratorColor
        {
            get
            {
                if (ownerIterator == 1)
                {
                    return new Color(0.749f, 0.651f, 0.145f);
                }

                if (ownerIterator == 2)
                {
                    return new Color(0.451f, 0.651f, 0.145f);
                }

                if (ownerIterator == 3)
                {
                    return new Color(0.82f, 0.031f, 0.031f);
                }

                if (ownerIterator == 4)
                {
                    return new Color(1f, 0.18f, 0.435f);
                }

                if (ownerIterator == 5)
                {
                    return new Color(0.471f, 0.471f, 0.471f);
                }

                return new Color(0.361f, 0.565f, 0.859f);
            }
        }

        public Outspector(AbstractCreature abstractCreature, World world)
        ...
        public override void Update(bool eu)
        {
            ...
            if (AI.behavior == OutspectorAI.Behavior.InspectArea)
            {
            ...
            if (base.Consious)
            {
                if (Random.value > 0.6f)
                {
                    ...
                    bool flag = true;
                    if (base.graphicsModule != null && (base.graphicsModule as OutspectorGraphics).blinks[activeEye] > 0f)
                    {
                        flag = false;
                    }
        ...
        public override void InitiateGraphicsModule()
        {
            ...
            if (base.graphicsModule == null)
            {
                base.graphicsModule = new OutspectorGraphics(this);
            }
        ...
        private void UpdateCoralNeuronSystem()
        {
            ...
            {
                for (int i = 0; i < (base.graphicsModule as OutspectorGraphics).mycelia.Length; i++)
                {
                    neuronSystem.mycelia.Remove((base.graphicsModule as OutspectorGraphics).mycelia[i]);
                }
            }
            neuronSystem = null;
            ...
            if (base.graphicsModule != null)
            {
                (base.graphicsModule as OutspectorGraphics).UpdateNeuronSystemForMycelia();
            }
        }

        public bool CanGrabCritter(AbstractCreature creature)
        {
            if (base.abstractCreature != creature && creature.realizedCreature != null && creature.creatureTemplate.TopAncestor().type != CreatureTemplateType.Outspector)
            {
        ...
        private void Act()
        {
            ...
            for (int l = 0; l < headCount(); l++)
            {
                if ((base.State as OutspectorState).headHealth[l] <= 0f)
                {
                    heads[l].limp = true;
                ...
                else if (headWantToGrabChunk[l] != null)
                {
                    if (headWantToGrabChunk[l].owner.room != room)
                    {
                        ...
                        Custom.Log("Outspector stopped grab due to not the same room");
                        continue;
                    }
                    if (headWantToGrabChunk[l].owner is SSOracleSwarmer && headWantToGrabChunk[l].owner.grabbedBy.Count == 0)
                    {
                        headWantToGrabChunk[l] = null;
                        heads[l].floatGrabDest = AI.HeadGoal(l);
                        Custom.Log("Outspector stopped neuron grab due to release");
                        continue;
                    }
                            ...
                            if (num7 == room.abstractRoom.index)
                            {
                                ...
                                if (!heads[l].floatGrabDest.HasValue)
                                {
                                    ...
                                    if (heads[l].Tip.vel.magnitude > 25f)
                                    {
                                        ...
                                        if (a.x != 0 || a.y != 0)
                                        {
                                            Custom.Log($"Outspector throw weapon {headGrabChunk[l].owner}");
                ...
                {
                    if (headCuriosityFocus[l] is Creature && (headCuriosityFocus[l] as Creature).abstractCreature.creatureTemplate.type == CreatureTemplateType.Outspector)
                    {
                        heads[l].floatGrabDest = Vector2.Lerp(heads[l].Tip.pos, (headCuriosityFocus[l] as Outspector).heads[l].Tip.pos, 0.25f);
                    }
                    else
                    {
            ...
            else
            {
                ...
                float t = Mathf.InverseLerp(2f, 16f, (base.State as OutspectorState).Wingnumber);
        ...
        public override void Violence(BodyChunk source, Vector2? directionAndMomentum, BodyChunk hitChunk, Appendage.Pos hitAppendage, DamageType type, float damage, float stunBonus)
        {
            ...
            if (source != null && source.owner != null && source.owner is Weapon && (source.owner as Weapon).thrownBy != null)
            {
                ...
                {
                    (base.State as OutspectorState).headHealth[hitAppendage.appendage.appIndex] -= damage;
                    damage = 0.12f;
                }
            }
            ...
            AI.behavior = OutspectorAI.Behavior.Idle;
        ...
        {
            if ((base.State as OutspectorState).headHealth[index] > 0f)
            {
