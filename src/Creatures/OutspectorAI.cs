...
namespace VoidTemplate.Creatures
{
    public class OutspectorAI : ArtificialIntelligence, IUseItemTracker, IUseARelationshipTracker, ILookingAtCreatures
    {
                ...
                if (currentRelationship.intensity >= 1f)
                {
                    ...
                    if (realizedCreature2.abstractCreature.creatureTemplate.TopAncestor().type != CreatureTemplate.Type.DaddyLongLegs)
                    {
                        if (currentRelationship.intensity >= 1f)
                        {
                            ...
                            {
                                if (myOutspector.heads[num2].Tip.vel.magnitude is < 1f or > 4f)
                                {
        ...
        public override void Update()
        {
            ...
            if (behavior == Behavior.Idle)
            {
                ...
                else
                {
                    ...
                    {
                        WorldCoordinate worldCoordinate = new(creature.pos.room, Random.Range(0, myOutspector.room.TileWidth), Random.Range(0, myOutspector.room.TileHeight), -1);
        ...
        public void PopulateHeadGoals(int posamount)
        {
            ...
            {
                WorldCoordinate worldCoordinate = new(myOutspector.room.abstractRoom.index, Random.Range(0, myOutspector.room.TileWidth), Random.Range(0, myOutspector.room.TileHeight), -1);
        ...
        public void PopulateHeadSpecialGoals(int posamount)
        {
            foreach (ZapCoil zapCoil in myOutspector.room.zapCoils)
            {
                for (int i = 0; i < posamount; i++)
                {
                    WorldCoordinate worldCoordinate = new(myOutspector.room.abstractRoom.index, Random.Range(zapCoil.rect.left - 30, zapCoil.rect.right + 30), Random.Range(zapCoil.rect.bottom - 30, zapCoil.rect.top + 30), -1);
            ...
            foreach (PlacedObject placedObject in myOutspector.room.roomSettings.placedObjects)
            {
                if (placedObject.type == PlacedObject.Type.CoralCircuit)
                {
                    ...
                    {
                        WorldCoordinate worldCoordinate2 = new(myOutspector.room.abstractRoom.index, (int)Random.Range(placedObject.pos.x - 60f, placedObject.pos.x + 60f), (int)Random.Range(placedObject.pos.y - 60f, placedObject.pos.y + 60f), -1);
                ...
                else
                {
                    ...
                    {
                        WorldCoordinate worldCoordinate3 = new(myOutspector.room.abstractRoom.index, (int)Random.Range(placedObject.pos.x - 80f, placedObject.pos.x + 80f), (int)Random.Range(placedObject.pos.y - 80f, placedObject.pos.y + 80f), -1);
