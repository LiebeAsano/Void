...
{
    public class OutspectorB : Creature, PhysicalObject.IHaveAppendages, IOwnProjectedCircles
    {
        public class OutspectorBState : HealthState
        {
            ...
            public OutspectorBState(AbstractCreature creature)
                : base(creature)
            {
        ...
        public OutspectorB(AbstractCreature abstractCreature, World world)
        ...
        public override void Update(bool eu)
        {
            ...
            if (base.Consious)
            {
                if (Random.value > 0.6f)
                {
                    ...
                    bool flag = true;
                    if (base.graphicsModule != null && (base.graphicsModule as OutspectorBGraphics).blinks[activeEye] > 0f)
                    {
                        flag = false;
                    }
        ...
        public override void InitiateGraphicsModule()
        {
            ...
            if (base.graphicsModule == null)
            {
                base.graphicsModule = new OutspectorBGraphics(this);
            }
        ...
        private void UpdateCoralNeuronSystem()
        {
            ...
            {
                for (int i = 0; i < (base.graphicsModule as OutspectorBGraphics).mycelia.Length; i++)
                {
                    neuronSystem.mycelia.Remove((base.graphicsModule as OutspectorBGraphics).mycelia[i]);
                }
            }
            neuronSystem = null;
            ...
            if (base.graphicsModule != null)
            {
                (base.graphicsModule as OutspectorBGraphics).UpdateNeuronSystemForMycelia();
            }
        }

        public bool CanGrabCritter(AbstractCreature creature)
        {
            if (base.abstractCreature != creature && creature.realizedCreature != null && creature.creatureTemplate.TopAncestor().type != CreatureTemplateType.OutspectorB)
            {
        ...
        private void Act()
        {
            ...
            for (int l = 0; l < headCount(); l++)
            {
                if ((base.State as OutspectorBState).headHealth[l] <= 0f)
                {
                    heads[l].limp = true;
                ...
                else if (headWantToGrabChunk[l] != null)
                {
                    if (headWantToGrabChunk[l].owner.room != room)
                    {
                        ...
                        Custom.Log("OutspectorB stopped grab due to not the same room");
                        continue;
                    }
                    if (headWantToGrabChunk[l].owner is SSOracleSwarmer && headWantToGrabChunk[l].owner.grabbedBy.Count == 0)
                    {
                        headWantToGrabChunk[l] = null;
                        heads[l].floatGrabDest = AI.HeadGoal(l);
                        Custom.Log("OutspectorB stopped neuron grab due to release");
                        continue;
                    }
                ...
                {
                    if (headCuriosityFocus[l] is Creature && (headCuriosityFocus[l] as Creature).abstractCreature.creatureTemplate.type == CreatureTemplateType.OutspectorB)
                    {
                        heads[l].floatGrabDest = Vector2.Lerp(heads[l].Tip.pos, (headCuriosityFocus[l] as OutspectorB).heads[l].Tip.pos, 0.25f);
                    }
                    else
                    {
            ...
            else
            {
                ...
                float t = Mathf.InverseLerp(2f, 16f, (base.State as OutspectorBState).Wingnumber);
        ...
        public override void Violence(BodyChunk source, Vector2? directionAndMomentum, BodyChunk hitChunk, Appendage.Pos hitAppendage, DamageType type, float damage, float stunBonus)
        {
            ...
            if (source != null && source.owner != null && source.owner is Weapon && (source.owner as Weapon).thrownBy != null)
            {
                ...
                {
                    (base.State as OutspectorBState).headHealth[hitAppendage.appendage.appIndex] -= damage;
                    damage = 0.12f;
                }
            }
            ...
            AI.behavior = OutspectorBAI.Behavior.Idle;
        ...
        {
            if ((base.State as OutspectorBState).headHealth[index] > 0f)
            {
