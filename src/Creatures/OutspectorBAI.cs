...
{
    public class OutspectorBAI : ArtificialIntelligence, IUseItemTracker, IUseARelationshipTracker, ILookingAtCreatures
    {
        ...
        public OutspectorB myOutspectorB => creature.realizedCreature as OutspectorB;

        public OutspectorBAI(AbstractCreature creature, World world)
            : base(creature, world)
        {
            myOutspectorB.AI = this;
        ...
        CreatureTemplate.Relationship IUseARelationshipTracker.UpdateDynamicRelationship(RelationshipTracker.DynamicRelationship dRelation)
        {
            ...
            if (currentRelationship.type == CreatureTemplate.Relationship.Type.Uncomfortable || dRelation.trackerRep.representedCreature.creatureTemplate.type == CreatureTemplateType.OutspectorB)
            {
                if (dRelation.trackerRep.VisualContact)
                {
                    if (dRelation.trackerRep.representedCreature.realizedCreature != null && !dRelation.trackerRep.representedCreature.realizedCreature.dead)
                    {
                        ...
                        if (realizedCreature.grasps != null && realizedCreature.grasps.Length != 0)
                        {
                            for (int i = 0; i < realizedCreature.grasps.Length; i++)
                            {
                                ...
                                currentRelationship.intensity = 1f;
                                if (!myOutspectorB.safariControlled)
                                {
                                    myOutspectorB.anger += 0.09f;
                                    OrderAHeadToGrabObject(realizedCreature.grasps[i].grabbed);
                                }
                            }
                        }

                        if (!myOutspectorB.safariControlled && dRelation.trackerRep.representedCreature.creatureTemplate.type != CreatureTemplateType.OutspectorB && myOutspectorB.DangerousThrowLocations.Count > 0 && Random.value < 0.1f)
                        {
                            foreach (Vector2 dangerousThrowLocation in myOutspectorB.DangerousThrowLocations)
                            {
                    ...
                    if (Vector2.Distance(myOutspectorB.mainBodyChunk.pos, dRelation.trackerRep.lastSeenCoord.Tile.ToVector2() * 20f) < 100f && currentRelationship.intensity + 0.09f < 1f)
                    {
            ...
            {
                if (myOutspectorB.activeEye != -1 && !myOutspectorB.safariControlled && Vector2.Distance(myOutspectorB.heads[myOutspectorB.activeEye].Tip.pos, dRelation.trackerRep.lastSeenCoord.Tile.ToVector2() * 20f) < 500f)
                {
                    myOutspectorB.anger += 0.19f;
                    if (myOutspectorB.anger > 1f)
                    {
                        if (myOutspectorB.anger > 2f)
                        {
                            myOutspectorB.anger = 2f;
                        }
                ...
                else if (!myOutspectorB.safariControlled)
                {
                    myOutspectorB.anger -= 0.001f;
                    if (myOutspectorB.anger < 0f)
                    {
                        myOutspectorB.anger = 0f;
                    }
                }
            }
            else if (currentRelationship.type == CreatureTemplate.Relationship.Type.Eats && dRelation.trackerRep.VisualContact)
            {
                if (dRelation.trackerRep.representedCreature.realizedCreature.dead && myOutspectorB.anger <= 0f)
                {
                ...
                if (!myOutspectorB.abstractCreature.controlled)
                {
                    myOutspectorB.anger += 0.19f;
                }
                ...
                if (currentRelationship.intensity >= 1f)
                {
                    ...
                    if (realizedCreature2.abstractCreature.creatureTemplate.TopAncestor().type != CreatureTemplate.Type.DaddyLongLegs)
                    {
                        ...
                        {
                            int num2 = -1;
                            for (int j = 0; j < OutspectorB.headCount(); j++)
                            {
                                if (!myOutspectorB.HeadsCrippled(j) && myOutspectorB.headGrabChunk[j] != null && myOutspectorB.headGrabChunk[j].owner == realizedCreature2)
                                {
                                    num2 = j;
                                    break;
                                }
                            ...
                            else
                            {
                                if (myOutspectorB.heads[num2].Tip.vel.magnitude < 1f || myOutspectorB.heads[num2].Tip.vel.magnitude > 4f)
                                {
                                    ...
                                    realizedCreature2.firstChunk.pos = myOutspectorB.heads[num2].Tip.pos;
                                    realizedCreature2.firstChunk.vel = myOutspectorB.heads[num2].Tip.vel;
                                }

                                float target = Custom.VecToDeg(Custom.DirVec(myOutspectorB.mainBodyChunk.pos, realizedCreature2.firstChunk.pos));
                                bool flag = false;
                                float num3 = 2000f;
                                for (int k = 0; k < myOutspectorB.DangerousThrowLocations.Count; k++)
                                {
                                    Vector2 vector = Vector2.Lerp(realizedCreature2.firstChunk.pos, myOutspectorB.DangerousThrowLocations[k], 0.8f);
                                    if (Random.value < 0.85f && Vector2.Distance(myOutspectorB.mainBodyChunk.pos, myOutspectorB.DangerousThrowLocations[k]) < num3 && myOutspectorB.room.RayTraceTilesForTerrain((int)(realizedCreature2.firstChunk.pos.x / 20f), (int)(realizedCreature2.firstChunk.pos.y / 20f), (int)(vector.x / 20f), (int)(vector.y / 20f)))
                                    {
                                        num3 = Vector2.Distance(myOutspectorB.mainBodyChunk.pos, myOutspectorB.DangerousThrowLocations[k]);
                                        flag = true;
                                        target = Custom.VecToDeg(Custom.DirVec(realizedCreature2.firstChunk.pos, myOutspectorB.DangerousThrowLocations[k]));
                                    }
                                }
                                ...
                                if (!flag && !myOutspectorB.room.RayTraceTilesForTerrain((int)(realizedCreature2.firstChunk.pos.x / 20f), (int)(realizedCreature2.firstChunk.pos.y / 20f), (int)(realizedCreature2.firstChunk.pos.x + realizedCreature2.firstChunk.vel.x * 3f / 20f), (int)(realizedCreature2.firstChunk.pos.y + realizedCreature2.firstChunk.vel.y * 3f / 20f)) && (realizedCreature2.firstChunk.vel.magnitude > 50f || (Random.value < 0.5f && Mathf.DeltaAngle(Custom.VecToDeg(realizedCreature2.firstChunk.vel), target) < num4 && realizedCreature2.firstChunk.vel.magnitude > 30f)))
                                {
                                    ...
                                    myOutspectorB.headGrabChunk[num2] = null;
                                    myOutspectorB.room.PlaySound(SoundID.Vulture_Peck, myOutspectorB.heads[num2].Tip.pos);
                                }
        ...
        {
            base.Update();
            if (myOutspectorB.LickedByPlayer != null)
            {
                base.tracker.SeeCreature(myOutspectorB.LickedByPlayer.abstractCreature);
            }
            ...
            if (myOutspectorB.safariControlled)
            {
                if (controlledAnger)
                {
                    myOutspectorB.anger = Mathf.Lerp(myOutspectorB.anger, 1f, 0.12f);
                }
                else
                {
                    myOutspectorB.anger = Mathf.Lerp(myOutspectorB.anger, 0f, 0.12f);
                }

                if (myOutspectorB.inputWithDiagonals.HasValue && myOutspectorB.inputWithDiagonals.Value.jmp && !myOutspectorB.lastInputWithDiagonals.Value.jmp)
                {
            ...
            if (AttentionGrabberTimer > 0f)
            {
                ...
                if (AttentionGrabberTimer == 0f)
                {
                    ...
                    {
                        if (!myOutspectorB.HeadWeaponized(i))
                        {
                            myOutspectorB.headCuriosityFocus[i] = FirstTimeAttentionGrabber;
                        }
                    }
            ...
            if (HeadIdler <= 0)
            {
                HeadFindNewMoreInterestingGoal(Random.Range(0, OutspectorB.headCount()));
            ...
            if (behavior == Behavior.EscapeRain || myOutspectorB.AllHeadsCrippled())
            {
            ...
            if (behavior == Behavior.Idle)
            {
                if (this.newIdlePosCounter < 1 && this.myOutspectorB.room.aimap.getTerrainProximity(this.myOutspectorB.abstractCreature.pos) > 4 && this.myOutspectorB.abstractCreature.abstractAI.destination.room == this.myOutspectorB.abstractCreature.pos.room && Vector2.Distance(this.myOutspectorB.abstractCreature.abstractAI.destination.Tile.ToVector2(), this.myOutspectorB.abstractCreature.pos.Tile.ToVector2()) < 80f)
                {
                    Debug.Log("OutspectorB triggered inspection mode");
                    AllHeadGoals.Clear();
                    PopulateInspectionModeHeadGoals(50);
                    PopulateHeadSpecialGoals(15);
                    behavior = Behavior.InspectArea;
                    creature.abstractAI.SetDestination(myOutspectorB.abstractCreature.abstractAI.destination);
                ...
                else
                {
                    ...
                    else if (base.pathFinder.GetDestination.room == this.creature.pos.room && base.pathFinder.GetDestination.TileDefined && this.myOutspectorB.room.aimap.getTerrainProximity(base.pathFinder.GetDestination) < 1)
                    {
                        WorldCoordinate worldCoordinate2 = base.pathFinder.GetDestination + Custom.fourDirections[UnityEngine.Random.Range(0, 4)];
                        if (base.pathFinder.CoordinateReachableAndGetbackable(worldCoordinate2) && this.myOutspectorB.room.aimap.getTerrainProximity(worldCoordinate2) > this.myOutspectorB.room.aimap.getTerrainProximity(base.pathFinder.GetDestination))
                        {
                            this.creature.abstractAI.SetDestination(worldCoordinate2);
                        }

                        if (base.pathFinder.GetDestination.room == creature.pos.room && base.pathFinder.GetDestination.TileDefined && base.pathFinder.GetDestination.Tile.FloatDist(creature.pos.Tile) < 3f && myOutspectorB.room.VisualContact(creature.pos.Tile, base.pathFinder.GetDestination.Tile))
                        {
                        ...
                        {
                            foreach (ZapCoil zapCoil in myOutspectorB.room.zapCoils)
                            {
                                ...
                                rect.bottom -= 5;
                                rect.top += 5;
                                if (Custom.InsideRect(myOutspectorB.room.GetTilePosition(pos), rect))
                                {
                ...
                if (behavior == Behavior.InspectArea)
                {
                    ...
                    {
                        Debug.Log("OutspectorB left inspection mode");
                        behavior = Behavior.Idle;
                        AllHeadGoals.Clear();
        ...
        public override float VisualScore(Vector2 lookAtPoint, float targetSpeed)
        {
            ...
            if (myOutspectorB.activeEye == -1)
            {
                return 0f;
            }

            if ((myOutspectorB.State as OutspectorB.OutspectorBState).headHealth[myOutspectorB.activeEye] > 0f)
            {
                Tentacle tentacle = myOutspectorB.heads[myOutspectorB.activeEye];
        ...
        {
            Vector2 pos = myOutspectorB.heads[headIndex].Tip.pos;
            pos = Vector2.Lerp(pos, myOutspectorB.mainBodyChunk.pos, Random.Range(0.4f, 0.9f));
            pos += myOutspectorB.mainBodyChunk.vel * 10f;
            float a = Mathf.InverseLerp(800f, 60f, Vector2.Distance(pos, Goal));
            a = Mathf.Lerp(a, 0f, Mathf.InverseLerp(0f, 100f, Vector2.Distance(myOutspectorB.mainBodyChunk.pos, Goal)));
        ...
        public void HeadFindNewMoreInterestingGoal(int headIndex)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                if (creatureLooker.lookCreature != null && creatureLooker.lookCreature.VisualContact && creatureLooker.lookCreature.representedCreature.realizedCreature != null && creatureLooker.lookCreature.dynamicRelationship != null && creatureLooker.lookCreature.dynamicRelationship.currentRelationship.intensity > 0.6f && (Vector2.Distance(creatureLooker.lookCreature.representedCreature.realizedCreature.firstChunk.pos, myOutspectorB.heads[headIndex].Tip.pos) < 60f || Vector2.Distance(creatureLooker.lookCreature.representedCreature.realizedCreature.firstChunk.pos, myOutspectorB.firstChunk.pos) < 128f))
                {
                    myOutspectorB.headCuriosityFocus[headIndex] = creatureLooker.lookCreature.representedCreature.realizedCreature;
                }
                else if (HeadGoalInterest(headIndex, HeadGoal(headIndex)) < HeadGoalInterest(headIndex, vector))
                {
                    myOutspectorB.headCuriosityFocus[headIndex] = null;
        ...
        public void PopulateHeadSpecialGoals(int posamount)
        {
            ...
            foreach (PlacedObject placedObject in myOutspectorB.room.roomSettings.placedObjects)
            {
        ...
        public void PopulateInspectionModeHeadGoals(int posamount)
        {
            for (int i = 0; i < posamount; i++)
            {
                WorldCoordinate pos = myOutspectorB.abstractCreature.pos;
                pos.x += Random.Range(-20, 20);
                pos.y += Random.Range(-20, 20);
                if (myOutspectorB.room.IsPositionInsideBoundries(pos.Tile) && !myOutspectorB.room.GetTile(pos.Tile).Solid)
                {
        ...
        public void GrabAttentionWithObject(PhysicalObject inputObject)
        {
            if (myOutspectorB.activeEye != -1)
            {
                myOutspectorB.headCuriosityFocus[myOutspectorB.activeEye] = inputObject;
                AttentionGrabberTimer = OutspectorB.attentionDelayMax;
            }
        }
        ...
        public void OrderAHeadToGrabObject(PhysicalObject Object)
        {
            int i = 0;
            int num = 0;
            for (; i < OutspectorB.headCount(); i++)
            {
                if (!myOutspectorB.HeadsCrippled(i) && !myOutspectorB.HeadWeaponized(i) && Vector2.Distance(myOutspectorB.heads[i].Tip.pos, Object.firstChunk.pos) < Vector2.Distance(myOutspectorB.heads[num].Tip.pos, Object.firstChunk.pos))
                {
                    num = i;
                }
            }

            if (myOutspectorB.headWantToGrabChunk[num] == null && myOutspectorB.headGrabChunk[num] == null)
            {
                myOutspectorB.headWantToGrabChunk[num] = Object.firstChunk;
            }
        }
    }
}
