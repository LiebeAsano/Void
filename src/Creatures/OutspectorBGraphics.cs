...
{
    public class OutspectorBGraphics : GraphicsModule, IDrawable, IOwnMycelia
    {
        // Token: 0x060022E4 RID: 8932 RVA: 0x002BFE28 File Offset: 0x002BE028
        public OutspectorBGraphics(PhysicalObject ow) : base(ow, false)
        {
            this.ropeGraphics = new OutspectorBGraphics.OutspectorBHeadRopeGraphics[OutspectorB.headCount()];
            for (int i = 0; i < OutspectorB.headCount(); i++)
            {
                this.ropeGraphics[i] = new OutspectorBGraphics.OutspectorBHeadRopeGraphics(this, i);
            }
            this.cullRange = 300f;
            this.JawAngleWiggler = new float[OutspectorB.headCount()];
            this.mycelia = new Mycelium[(int)(5f + 5f * this.myOutspectorB.room.world.game.SeededRandom(this.myOutspectorB.abstractCreature.ID.RandomSeed))];
            this.blinks = new float[OutspectorB.headCount()];
            this.wingBodyParts = new GenericBodyPart[(this.myOutspectorB.State as OutspectorB.OutspectorBState).Wingnumber];
            for (int j = 0; j < this.wingBodyParts.Length; j++)
            {
                this.wingBodyParts[j] = new GenericBodyPart(this, 15f, 0.2f, 0.1f, this.myOutspectorB.firstChunk);
            }
            this.wingBodyPartDistance = 75f;
            for (int k = 0; k < this.mycelia.GetLength(0); k++)
            {
                this.mycelia[k] = new Mycelium(this.myOutspectorB.neuronSystem, this, k, Mathf.Lerp(120f, 300f, this.myOutspectorB.room.world.game.SeededRandom(this.myOutspectorB.abstractCreature.ID.RandomSeed + k)), this.myOutspectorB.mainBodyChunk.pos);
                this.mycelia[k].useStaticCulling = false;
                this.mycelia[k].color = this.myOutspectorB.bodyColor;
                this.bodyRotation = 0f;
            }
        }

        private OutspectorB myOutspectorB
        {
            get
            {
                return base.owner as OutspectorB;
            }
        }
        ...
        public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
        {
            ...
            for (int j = 0; j < this.SpritesTotal_wings; j++)
            {
                ...
                sLeaser.sprites[this.SpritesBegin_wings + j].scaleX = Mathf.Lerp(3.6f, 5f, this.myOutspectorB.room.world.game.SeededRandom(this.myOutspectorB.abstractCreature.ID.RandomSeed));
            ...
            int num = 0;
            for (int k = 0; k < OutspectorB.headCount(); k++)
            {
        ...
        public override void DrawSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
        {
            ...
            if (this.culled || this.myOutspectorB.room == null)
            {
                return;
            }
            Vector2 pos = this.myOutspectorB.mainBodyChunk.pos;
            ...
            Color bodyColor = this.myOutspectorB.bodyColor;
            ...
            for (int j = 0; j < this.SpritesTotal_wings; j++)
            {
                ...
                sLeaser.sprites[this.SpritesBegin_wings + j].scaleX = Mathf.Lerp(0f, Mathf.Lerp(3.6f, 6f, this.myOutspectorB.room.world.game.SeededRandom(this.myOutspectorB.abstractCreature.ID.RandomSeed)) * Mathf.InverseLerp(1f, 0.8f, this.myOutspectorB.squeezeFac), Mathf.InverseLerp(1f, 0.55f, Vector2.Distance(this.wingBodyParts[j].pos, this.myOutspectorB.firstChunk.pos + b) / this.wingBodyPartDistance));
                this.wingflapCounters[j] += 0.02f;
                this.wingflapCounters[j] += this.findWingFlapIntensity(j, this.myOutspectorB.mainBodyChunk.vel * -1f) / 8f;
                this.wingflapCounters[j] += this.findWingFlapIntensity(j, this.myOutspectorB.flyingPower) * 3f;
            ...
            for (int l = 0; l < this.SpritesTotal_Core; l++)
            {
                ...
                sLeaser.sprites[l].scale = 1.3f + Mathf.Sin(this.myOutspectorB.lightpulse + (float)l / (float)l) / 4f;
                sLeaser.sprites[l].color = this.myOutspectorB.bodyColor;
            }
            int num3 = 0;
            for (int m = 0; m < OutspectorB.headCount(); m++)
            {
                Vector2 lastPos = this.myOutspectorB.heads[m].tChunks[this.myOutspectorB.heads[m].tChunks.Length - 2].lastPos;
                Vector2 lastPos2 = this.myOutspectorB.heads[m].Tip.lastPos;
                ...
                sLeaser.sprites[this.SpritesBegin_heads + num3 + 1].color = this.myOutspectorB.bodyColor;
                ...
                vector *= 10f;
                Color blue;
                if (this.myOutspectorB.activeEye == m && !this.myOutspectorB.HeadsCrippled(this.myOutspectorB.activeEye))
                {
                    blue = Color.blue;
                }
                else
                ...
                if ((this.myOutspectorB.State as OutspectorB.OutspectorBState).headHealth[m] > 0f)
                {
                    sLeaser.sprites[this.SpritesBegin_Eye(m)].scaleX = Mathf.Lerp(Mathf.Lerp(sLeaser.sprites[this.SpritesBegin_Eye(m)].scaleX, Mathf.Lerp(0.525f, 0.225f, this.myOutspectorB.anger), 0.15f), 0.1f, this.blinks[m]);
                    sLeaser.sprites[this.SpritesBegin_Eye(m)].scaleX = Mathf.Lerp(sLeaser.sprites[this.SpritesBegin_Eye(m)].scaleX, 0.25f + Mathf.Sin((float)this.myOutspectorB.blind) / 4f, Mathf.InverseLerp(0f, 500f, (float)this.myOutspectorB.blind));
                }
                else
                {
                ...
                sLeaser.sprites[this.SpritesBegin_heads + num3 + 2].color = this.myOutspectorB.bodyColor;
                ...
                sLeaser.sprites[this.SpritesBegin_heads + num3 + 3].color = this.myOutspectorB.bodyColor;
                ...
                sLeaser.sprites[this.SpritesBegin_heads + num3 + 4].color = this.myOutspectorB.bodyColor;
                ...
                sLeaser.sprites[this.SpritesBegin_heads + num3 + 5].color = this.myOutspectorB.bodyColor;
            ...
            {
                this.mycelia[n].UpdateColor(this.myOutspectorB.bodyColor, 0f, this.SpritesBegin_mycelium + n, sLeaser);
            }
        }
        ...
        public override void Update()
        {
            ...
            for (int i = 0; i < this.wingBodyParts.Length; i++)
            {
                ...
                this.wingBodyParts[i].ConnectToPoint(this.myOutspectorB.firstChunk.pos, this.wingBodyPartDistance, false, 0.3f, this.myOutspectorB.firstChunk.vel + a / 3f, 0.25f, 0.1f);
                if (Vector2.Distance(this.myOutspectorB.firstChunk.pos, this.wingBodyParts[i].pos) > this.wingBodyPartDistance * 1.1f || this.OwnerRoom.aimap.getAItile(this.myOutspectorB.firstChunk.pos).narrowSpace)
                {
                    this.wingBodyParts[i].pos = this.myOutspectorB.firstChunk.pos;
                }
            ...
            {
                this.JawAngle = new float[OutspectorB.headCount()];
            }
            for (int k = 0; k < OutspectorB.headCount(); k++)
            {
                if (this.blinks[k] > 0f)
                ...
                if ((this.myOutspectorB.State as OutspectorB.OutspectorBState).headHealth[k] <= 0f)
                {
                ...
                {
                    this.JawAngleWiggler[k] += Random.value * (0.1f + this.myOutspectorB.anger);
                    if (this.myOutspectorB.headWantToGrabChunk[k] != null)
                    {
                        this.JawAngle[k] = Mathf.Lerp(this.JawAngle[k], 25f, 0.15f) + Mathf.Sin(this.JawAngleWiggler[k]);
                    }
                    if (this.myOutspectorB.headGrabChunk[k] != null)
                    {
                ...
                if (this.myOutspectorB.dying > 0f && this.myOutspectorB.room.ViewedByAnyCamera(this.myOutspectorB.mainBodyChunk.pos, 900f))
                {
                    for (int l = 0; l < 15; l++)
                    {
                        int num = (int)Random.Range(0f, (float)this.myOutspectorB.heads[k].tChunks.Length);
                        this.myOutspectorB.room.AddObject(new OverseerEffect(this.myOutspectorB.heads[k].tChunks[num].pos, Custom.RNV() * Random.value * 0.1f, this.myOutspectorB.bodyColor, Mathf.Lerp(200f, 15f, this.myOutspectorB.dying), Mathf.Lerp(1.5f, 0.1f, this.myOutspectorB.dying)));
                    }
                    for (int m = 0; m < 8; m++)
                    {
                        int num2 = (int)Random.Range(0f, (float)this.myOutspectorB.heads[k].tChunks.Length);
                        this.myOutspectorB.room.AddObject(new Spark(this.myOutspectorB.heads[k].tChunks[num2].pos, this.myOutspectorB.mainBodyChunk.vel * 0.5f + Custom.RNV() * 14f * Random.value, this.myOutspectorB.bodyColor, null, 14, 21));
                    }
                }
            }
            if (this.myOutspectorB.Consious)
            {
                this.bodyRotation += this.myOutspectorB.flyingPower.x * 3f;
            }
        }

        public Vector2 ConnectionPos(int index, float timeStacker)
        {
            return this.myOutspectorB.mainBodyChunk.pos;
        }

        public Vector2 ResetDir(int index)
        {
            return this.myOutspectorB.mainBodyChunk.vel;
        }
        ...
        public void UpdateNeuronSystemForMycelia()
        {
            for (int i = 0; i < this.mycelia.Length; i++)
            {
                if (this.mycelia[i].system != this.myOutspectorB.neuronSystem)
                {
                    if (this.mycelia[i].system != null)
                    {
                        this.mycelia[i].system.mycelia.Remove(this.mycelia[i]);
                    }
                    if (this.myOutspectorB.neuronSystem != null)
                    {
                        this.myOutspectorB.neuronSystem.mycelia.Add(this.mycelia[i]);
                    }
                    this.mycelia[i].system = this.myOutspectorB.neuronSystem;
                }
            }
        }
        ...
        public override void Reset()
        {
            for (int i = 0; i < OutspectorB.headCount(); i++)
            {
                this.ropeGraphics[i].AddToPositionsList(0, this.myOutspectorB.mainBodyChunk.pos);
                this.ropeGraphics[i].AddToPositionsList(1, this.myOutspectorB.heads[i].Tip.pos);
                this.ropeGraphics[i].AlignAndConnect(2);
            }
            base.Reset();
            for (int j = 0; j < this.mycelia.Length; j++)
            {
                this.mycelia[j].Reset(this.myOutspectorB.mainBodyChunk.pos);
            }
        }
        ...
        public int SpritesTotal_wings
        {
            get
            {
                return (this.myOutspectorB.State as OutspectorB.OutspectorBState).Wingnumber;
            }
        }
        ...
        {
            return this.myOutspectorB.Rad(f) * Mathf.Pow(1f - Mathf.Lerp(this.myOutspectorB.lastDying, this.myOutspectorB.dying, timeStacker), 0.2f);
        }
        ...
        public OutspectorBGraphics.OutspectorBHeadRopeGraphics[] ropeGraphics;
        ...
        public class OutspectorBHeadRopeGraphics : RopeGraphic
        {
            public override void Update()
            {
                int listCount = 0;
                base.AddToPositionsList(listCount++, this.owner.myOutspectorB.heads[this.headNumber].FloatBase);
                for (int i = 0; i < this.owner.myOutspectorB.heads[this.headNumber].tChunks.Length; i++)
                {
                    for (int j = 1; j < this.owner.myOutspectorB.heads[this.headNumber].tChunks[i].rope.TotalPositions; j++)
                    {
                        base.AddToPositionsList(listCount++, this.owner.myOutspectorB.heads[this.headNumber].tChunks[i].rope.GetPosition(j));
                    }
                }
            ...
            {
                Vector2 vector = this.owner.myOutspectorB.mainBodyChunk.pos;
                ...
                for (int i = 0; i < this.segments.Length; i++)
                {
                    ...
                    (sLeaser.sprites[this.spriteOffset] as TriangleMesh).color = this.owner.myOutspectorB.bodyColor;
                    (sLeaser.sprites[this.spriteOffset] as TriangleMesh).alpha = this.owner.myOutspectorB.lightpulse;
            ...
            public override void MoveSegment(int segment, Vector2 goalPos, Vector2 smoothedGoalPos)
            {
                ...
                if (this.owner.myOutspectorB.room.GetTile(smoothedGoalPos).Solid && !this.owner.myOutspectorB.room.GetTile(goalPos).Solid)
                {
                    FloatRect floatRect = Custom.RectCollision(smoothedGoalPos, goalPos, this.owner.myOutspectorB.room.TileRect(this.owner.myOutspectorB.room.GetTilePosition(smoothedGoalPos)).Grow(3f));
            ...
            public OutspectorBHeadRopeGraphics(OutspectorBGraphics owner, int head) : base(40)
            {
                this.owner = owner;
            ...
            private OutspectorBGraphics owner;
