...
internal static class DisablePassage
{
	...
	private static void SleepAndDeathScreen_GetDataFromGame(MonoMod.Cil.ILContext il)
	{
			...
			x => x.MatchLdsfld<ModManager>(nameof(ModManager.MMF))))
		{
			...
			c.EmitDelegate<Func<bool, KarmaLadderScreen.SleepDeathScreenDataPackage, bool>>(static (orig, package) => orig && !(package.characterStats.name == VoidEnums.SlugcatID.Void));
		}
	...
	{
		if (self.saveState != null && (self.saveState.saveStateNumber == VoidEnums.SlugcatID.Void)) return; //no need in calling orig if it's void, because the button is not supposed to be here at all
