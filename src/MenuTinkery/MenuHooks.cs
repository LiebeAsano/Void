...
public static class MenuHooks
{
    ...
    public static SaveState GetVoidMenuSaveState(Menu.Menu menu)
    {
        if (menu?.manager?.rainWorld?.progression == null)
            return null;

        if (menu is StoryGameStatisticsScreen statistics &&
    ...
    {
        SaveState save = GetVoidMenuSaveState(menu);

        if (save?.GetVoidCatDead() == true)
            return true;

        return SaveManager.ExternalSaveData.VoidDead;
    }
    ...
    private static void SlugcatSelectMenu_ctor(On.Menu.SlugcatSelectMenu.orig_ctor orig, SlugcatSelectMenu self, ProcessManager manager)
    {
        ...
        self.pages[0].subObjects.Add(new LegalSpeedrunResetBox(
            self,
            self.pages[0],
            new(self.restartCheckbox.pos.x + 175 + SlugcatSelectMenu.GetRestartTextOffset(self.CurrLang), self.restartCheckbox.pos.y),
            SlugcatSelectMenu.GetRestartTextWidth(self.CurrLang) + 55,
            "Reset for legal speedrun"));
    }
    ...
    private static void SlugcatPageContinue_Update(ILContext il)
    {
        ...
        if (c.TryGotoNext(MoveType.After, x => x.MatchCallOrCallvirt(typeof(HUD.HUD).GetMethod(nameof(HUD.HUD.Update)))))
        {
            ...
            LogExErr("failed to find HUD.Update. no jump");
    }
    ...
    {
        if (self.owner?.menu is StoryGameStatisticsScreen statistics &&
            statistics.saveState?.saveStateNumber == VoidEnums.SlugcatID.Void)
        {
            SaveState save = statistics.saveState;

            bool dead = save.GetVoidCatDead() || SaveManager.ExternalSaveData.VoidDead;
            bool karma10 = save.deathPersistentSaveData.karmaCap == 10 || SaveManager.ExternalSaveData.VoidKarma11;
    ...
    private static void HideFoodPips(On.HUD.FoodMeter.orig_CharSelectUpdate orig, FoodMeter self)
    {
        ...
        if (self.hud.owner is not SlugcatSelectMenu.SlugcatPageContinue page ||
            page.slugcatNumber != VoidEnums.SlugcatID.Void)
            return;

        SaveState save = GetVoidMenuSaveState(page.menu);

        bool dead = save?.GetVoidCatDead() == true || SaveManager.ExternalSaveData.VoidDead;
        bool ending = save?.GetEndingEncountered() == true;
        bool treeEnding = save?.GetVoidEndingTree() == true;

        if (dead || ending || treeEnding)
    ...
    private static void HideKarmaAndFoodSplitterAndAddText(On.Menu.SlugcatSelectMenu.SlugcatPageContinue.orig_ctor orig, Menu.SlugcatSelectMenu.SlugcatPageContinue self, Menu.Menu menu, Menu.MenuObject owner, int pageIndex, SlugcatStats.Name slugcatNumber)
    {
        ...
        if (dead && karma10)
            ...
            text = TextIfSlugEnding;
        else
            text = TextIfEnding;

        var textlabel = new MenuLabel(
            menu,
            self,
            text.TranslateStringComplex(),
            new Vector2(-1000f, self.imagePos.y - 249f - 60f + VerticalOffset / 2f),
    ...
    public class LoadDataButton : SimpleButton
    {
        ...
        public LoadDataButton(StoryGameStatisticsScreen menu, MenuObject owner, string displayText, Action clickAction, Vector2 pos, Vector2 size)
            : base(menu, owner, displayText, null, pos, size)
        {
            onClick = clickAction;
        }
    ...
    public class LegalSpeedrunResetBox : CheckBox
    {
        ...
        public LegalSpeedrunResetBox(SlugcatSelectMenu menu, MenuObject owner, Vector2 pos, float textWidth, string displayText, bool textOnRight = false)
            : base(menu, owner, default(SelfOwnCheckBox), pos, textWidth, displayText, null, textOnRight)
        {
