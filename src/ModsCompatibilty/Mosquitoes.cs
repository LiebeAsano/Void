...
namespace VoidTemplate.ModsCompatibilty
{
    public class MosquitoCompat
    {
        ...
        public static void Init()
        {
            ...
            InitHooks(mosquitoType, mosquitoGraphicsType);

        }

        private static void InitHooks(Type mosquitoType, Type mosquitoGraphicsType)
        {
        ...
        private static readonly Delegate StickIntoChunkHook =
            (Action<Mosquito, PhysicalObject, int> orig, Mosquito self, PhysicalObject otherObject, int otherChunk) =>
            {
        ...
        private static readonly Delegate UpdateHook =
            (Action<Mosquito, bool> orig, Mosquito self, bool eu) =>
            {
                orig(self, eu);
        ...
        private static readonly Delegate ApplyPaletteHook =
            (Action<MosquitoGraphics, RoomCamera.SpriteLeaser, RoomCamera, RoomPalette> orig, MosquitoGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, RoomPalette palette) =>
            {
        ...
        private static void InfectMosquito(object mosquito)
        {
            if (!infectedMosquitoes.TryGetValue(mosquito, out _))
            {
                var deadProperty = mosquito.GetType().GetProperty("dead");
                if (deadProperty != null && !(bool)deadProperty.GetValue(mosquito))
                {
