...
internal class Warp : UpdatableAndDeletable
{
    ...
    string TargetRegionAcronym => TargetRoom.Split('_')[0];
    #endregion
            ...
            case State.awaiting:
                ...
                && PositionWithinPoly(TriggerZone, realizedPlayerInRoom.mainBodyChunk.pos)))
                {
                    ...
                    WorldLoader worldLoader = new(game: room.game,
                        ...
                        //this may be wron, maybe there is no need to wrap
                        worldName: Region.GetProperRegionAcronym(room.game.StoryCharacter, TargetRegionAcronym),

                        region: room.game.overWorld.GetRegion(TargetRegionAcronym),
                        setupValues: room.game.setupValues);
                    threadedLoading = new(warp: this, loader: worldLoader);
    ...
    class ThreadedLoading
    {
        ...
        public ThreadedLoading(WorldLoader loader, Warp warp)
        {
