			...
			if (self.eatMeat > 20)
			{
				if (ModManager.MSC)
				{
					...
					if (SlugcatStats.SlugcatCanMaul(self.SlugCatClass) && creature is Vulture vulture && self.grasps[graspIndex].grabbedChunk.index == 4 && vulture.abstractCreature.state is Vulture.VultureState vultureState && vultureState.mask)
					{
						...
						for (int i = Random.Range(8, 14); i >= 0; i--)
						{
				...
				if (self.graphicsModule != null && (self.grasps[graspIndex].grabbed as Creature).State.meatLeft > 0 && self.FoodInStomach < self.MaxFoodInStomach)
				{
					...
					if (self.eatMeat > 40 && self.eatMeat % 15 == 3)
					{
						...
						{
							self.room.AddObject(new WaterDrip(Vector2.Lerp(self.grasps[graspIndex].grabbedChunk.pos, self.mainBodyChunk.pos, Random.value) + self.grasps[graspIndex].grabbedChunk.rad * Custom.RNV() * UnityEngine.Random.value, Custom.RNV() * 6f * UnityEngine.Random.value + Custom.DirVec(vector, (self.mainBodyChunk.pos + (self.graphicsModule as PlayerGraphics).head.pos) / 2f) * 7f * UnityEngine.Random.value + Custom.DegToVec(Mathf.Lerp(-90f, 90f, UnityEngine.Random.value)) * UnityEngine.Random.value * self.EffectiveRoomGravity * 7f, false));
						}
            ...
            if (self.eatMeat > 20)
            {
                if (ModManager.MSC)
                {
                    ...
                    if (SlugcatStats.SlugcatCanMaul(self.SlugCatClass) && creature is Vulture vulture && self.grasps[graspIndex].grabbedChunk.index == 4 && vulture.abstractCreature.state is Vulture.VultureState vultureState && vultureState.mask)
                    {
                        ...
                        for (int i = Random.Range(8, 14); i >= 0; i--)
                        {
                            self.room.AddObject(new WaterDrip(Vector2.Lerp(self.grasps[graspIndex].grabbedChunk.pos, self.mainBodyChunk.pos, Random.value) + self.grasps[graspIndex].grabbedChunk.rad * Custom.RNV() * UnityEngine.Random.value, Custom.RNV() * 6f * UnityEngine.Random.value + Custom.DirVec(creature.firstChunk.pos, (self.mainBodyChunk.pos + (self.graphicsModule as PlayerGraphics).head.pos) / 2f) * 7f * UnityEngine.Random.value + Custom.DegToVec(Mathf.Lerp(-90f, 90f, UnityEngine.Random.value)) * UnityEngine.Random.value * self.EffectiveRoomGravity * 7f, false));
                        }
                    }
                }
