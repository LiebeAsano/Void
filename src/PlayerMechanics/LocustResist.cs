...
namespace VoidTemplate.PlayerMechanics
{
    internal class LocustResist
    {
        ...
        {
            orig(self);
            Player player = self.target.abstractCreature.realizedCreature as Player;
            if (self.locusts.Count > self.maxLocusts - 5 && UnityEngine.Random.value < 0.2f && (player.slugcatStats.name == VoidEnums.SlugcatID.Void || player.slugcatStats.name == VoidEnums.SlugcatID.Viy))
            {
        ...
        {
            Creature realizedCreature = self.swarm.target.abstractCreature.realizedCreature;
