...
public static class ViyViolence
{
    ...
    {
        if (self is Player player && player.IsViy())
        {
            ...
            if (Damage >= self.Template.instantDeathDamageLimit * 2.5f || player.playerState.permanentDamageTracking >= 2.5f)
            {
                Debug.Log("&Damage" + player.playerState.permanentDamageTracking);
                self.Die();
            }
        }
        else
