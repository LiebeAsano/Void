...
namespace VoidTemplate.PortalForViy
{
    public class MSGhostHooks
    {
        ...
        {
            if (self.worldGhost.ghostID == MoreSlugcatsEnums.GhostID.MS && self.room.game.IsVoidWorld())
            {
                ...
                var warpID = WarpPoint.IdentifyingString(self.room.game, warp.data as WarpPoint.WarpPointData, self.room.abstractRoom);
