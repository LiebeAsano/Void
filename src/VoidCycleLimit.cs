...
public static class VoidCycleLimit
{
    ...
    public static int GetRemainingLimitedCycles(SaveState saveState)
    {
        int remainingCycles = GetVoidCycleLimit(saveState) - saveState.cycleNumber;
        return remainingCycles < 0 ? 0 : remainingCycles;
    }

    public static int GetDisplayCycleNumber(SaveState saveState)
    {
        return GetCycleLimitLifted(saveState) ? saveState.cycleNumber : GetRemainingLimitedCycles(saveState);
    }
    ...
    private static void SubregionTracker_Update(On.HUD.SubregionTracker.orig_Update orig, HUD.SubregionTracker self)
    {
        ...
        if (num != self.lastShownRegion && player.room != null && num != 0 && self.lastRegion == num && self.textPrompt.show == 0f)
        {
            if (player.room.world.game.IsVoidStoryCampaign())
            {
                ...
                for (int i = 0; i < player.room.warpPoints.Count; i++)
                {
                    if (player.room.warpPoints[i].timeWarpTearClosed <= 20)
                    {
                        flag = true;
                        break;
                    }
                ...
                if (self.counter > 80)
                {
                    if ((num > 1 || self.lastShownRegion == 0 || player.room.world.region.name != "SS" && player.room.world.region.name != "DM") &&
                        num < player.room.world.region.subRegions.Count)
                    {
                        if (self.showCycleNumber &&
                            player.room.game.IsStorySession &&
                            player.room.game.manager.menuSetup.startGameCondition == ProcessManager.MenuSetup.StoryGameInitCondition.Load)
                        {
                            int cycleNumber = YieldVoidCycleDisplayNumberWithPlayer(player, player.room.game.GetStorySession.saveState.cycleNumber);
                            string regionName = player.room.world.region.subRegions[num];

                            if (num < player.room.world.region.altSubRegions.Count && player.room.world.region.altSubRegions[num] != null)
                                regionName = player.room.world.region.altSubRegions[num];

                            self.textPrompt.AddMessage(
                                self.textPrompt.hud.rainWorld.inGameTranslator.Translate("Cycle") + " " +
                                cycleNumber + " ~ " +
                                self.textPrompt.hud.rainWorld.inGameTranslator.Translate(regionName),
                                0, 160, false, true);
                        }
                        else
                        {
    ...
    public static int YieldVoidCycleDisplayNumberWithPlayer(Player player, int originalCycleNumber)
    {
        ...
        if (saveState.saveStateNumber != VoidEnums.SlugcatID.Void || !OptionAccessors.PermaDeath)
